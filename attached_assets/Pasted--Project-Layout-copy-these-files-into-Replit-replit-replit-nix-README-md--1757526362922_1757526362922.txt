...
def weights_from_ranked_symbols(symbols: List[str]) -> List[Target]:
    ...
    s = sum(raw)
    return [Target(sym, w/s) for sym, w in zip(symbols, raw)]


def compute_orders(cash_to_invest: float, policy: List[Target] = DEFAULT_POLICY) -> Dict[str, float]:
    # returns {symbol: notional}
    return {t.symbol: round(cash_to_invest * t.weight, 2) for t in policy}
```
...
from .portfolio import compute_orders, weights_from_ranked_symbols, DEFAULT_POLICY
...
async def _fetch_ranked_symbols(limit: int = 5) -> list[str]:
    """Fetch ranked tickers from your website's existing signals endpoint.
    Expected JSON: { "ranked": ["NVDA","AAPL", ...] } or a bare list.
    """
    url = settings.ALGO_SIGNALS_URL
    headers = {"accept": "application/json"}
    if getattr(settings, "ALGO_SIGNALS_TOKEN", ""):
        headers["authorization"] = f"Bearer {settings.ALGO_SIGNALS_TOKEN}"
    try:
        async with httpx.AsyncClient(timeout=20) as c:
            r = await c.get(url, params={"limit": limit}, headers=headers)
            r.raise_for_status()
            data = r.json()
            if isinstance(data, dict) and isinstance(data.get("ranked"), list):
                return [str(t).upper() for t in data["ranked"]]
...
async def invest_auto(body: InvestReq):
    ...
    # Pull ranked stocks from your existing site (no UI changes)
    ranked = await _fetch_ranked_symbols(limit=5)
    policy = weights_from_ranked_symbols(ranked) if ranked else DEFAULT_POLICY

    orders = compute_orders(investable, policy)
    for sym, notional in orders.items():
        await broker.buy_fractional(sym, notional)
    # Response schema is unchanged so your front-end doesn't need edits
